                    ...
                    if line.receipt:
                        ...
                        if not '.pdf' in line.receipt.name.lower():
                            try:
                                ...
                                self.stdout.write(
                                    'Not able to convert receipt ' + line.receipt.path)
                                mail_admins(
                                    "File handling failed", "Not able to convert receipt" + line.receipt.path + "in kululasku-system.")
                                continue
                            if not im:
                                continue
                        ...
                        zf.write(receiptpath, 'liite_' + str(expense.id) + '_' +
                                 str(j).zfill(3) + os.path.splitext(str(receiptpath))[1])
                    j = j + 1
