...
class Command (BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    try:
      ...
      for expense in expenses:
        ...
        for line in lines:
          if line.receipt:
            ...
            zf.write(receiptpath, 'liite_' + str(expense.id) + '_' + str(j).zfill(3) + os.path.splitext(unicode(receiptpath))[1])
          j = j + 1
      ...
      import paramiko
      transport = paramiko.Transport(('***REMOVED***', 22))
      transport.connect(username='***REMOVED***', password='***REMOVED***')
